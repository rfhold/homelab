...
export class GiteaActRunner extends pulumi.ComponentResource {
  ...
  constructor(name: string, args: GiteaActRunnerArgs, opts?: pulumi.ComponentResourceOptions) {
    ...
    const runnerLabels = args.runnerLabels || "ubuntu-latest:docker://node:20-bookworm,ubuntu-22.04:docker://node:20-bookworm";
    const actRunnerImage = args.actRunnerImage || "gitea/act_runner:nightly";
    const dindImage = args.dindImage || "docker:23.0.6-dind";
    ...
    const dockerStorageConfig: StorageConfig = {
      size: args.dockerStorage?.size || "50Gi",
      storageClass: args.dockerStorage?.storageClass || args.dataStorage?.storageClass,
      accessModes: args.dockerStorage?.accessModes || ["ReadWriteOnce"],
      volumeMode: args.dockerStorage?.volumeMode,
      namespace: args.dockerStorage?.namespace,
      labels: args.dockerStorage?.labels,
      annotations: args.dockerStorage?.annotations,
      selector: args.dockerStorage?.selector,
      dataSource: args.dockerStorage?.dataSource,
    };
      ...
      spec: {
        ...
        volumeClaimTemplates: [
          ...
          },
          {
            ...
            spec: createPVCSpec(dockerStorageConfig),
          },
        ],
      },
    }, defaultResourceOptions);
