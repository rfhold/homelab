...
| Workload-label propagation | `uv run python -m unittest discover -s tests` when relevant and the existing environment is available | Tests prove checked source patterns, not provider rendering or cluster state. |
