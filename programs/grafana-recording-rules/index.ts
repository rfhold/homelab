...
for (const domain of fs.readdirSync(rulesDirectory).sort()) {
  for (const fileName of fs.readdirSync(path.join(rulesDirectory, domain)).filter(name => name.endsWith(".json")).sort()) {
    const args = JSON.parse(fs.readFileSync(path.join(rulesDirectory, domain, fileName), "utf8")) as grafana.alerting.v0alpha1.RecordingRuleArgs & { metadata: { uid: string } };
    new grafana.alerting.v0alpha1.RecordingRule(args.metadata.uid, args, { provider });
  }
}
