...
The homelab currently relies on passphrase-encrypted Pulumi config and program-specific Kubernetes Secrets without a single system of record for operator and application secrets. Adding OpenBao on `romulus` establishes an open-source secrets-management capability for internal services and operators while preserving the existing Pulumi-driven deployment model.

The first rollout needs to stay operationally narrow. The design therefore focuses on internal-only access, single-node deployment, Authentik-backed operator login, KV secret storage for people and services, and Transit support for Pulumi secrets-provider migration. It explicitly avoids HA, PKI, dynamic credentials, pod-side injection patterns, and a HashiCorp Vault deployment in the first version.

### Impact
- **Breaking changes**: none
- **Migration**: operators will need to initialize and manually unseal OpenBao, configure Authentik-backed login, create OpenBao policies and paths for Pulumi and workloads, and migrate Pulumi stacks from passphrase encryption to an OpenBao Transit-backed secrets provider before relying on OpenBao as the primary secret source
- **Cross-change dependencies**: none

### Non-goals
- High-availability OpenBao clustering
- Self-hosted or external auto-unseal
- Public internet exposure
- PKI issuance and dynamic database credentials
- External Secrets Operator, CSI-mounted secrets, or agent sidecar injection
- HashiCorp Vault deployment

### Rollback

Rollback consists of stopping OpenBao adoption without changing stable specs directly: keep existing Pulumi passphrase-based secret handling in place, remove the OpenBao deployment change, and leave existing Kubernetes Secret materialization paths unchanged until a future secrets-management approach is approved.
