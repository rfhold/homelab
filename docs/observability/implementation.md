...
- The node folder contains the `ROCm Metrics Exporter` dashboard for tracked ROCm exporter sources. It selects the stable `mimir` datasource, exporter job, instance, and GPU ID. Its source covers availability, health, identity, activity, package power, memory, clocks, and uncorrectable ECC totals.
