...
[`../../deploys/llama_cpp/templates/llama-cpp.service.j2`](../../deploys/llama_cpp/templates/llama-cpp.service.j2) starts one inventory-named Docker container with the model cache and AMD device mounts. It exposes inventory-owned `COSMOS_LABELS` in the systemd service environment for local systemd clients; source contains no catalog client, API, or privilege policy.
