...
Tracked provider configuration includes external OpenAI-compatible providers, Codex Proxy, a local vLLM embedding provider, and a local llama.cpp chat provider. The stable local aliases are `local-embedding`, mapped to `Qwen/Qwen3-Embedding-0.6B` at `qwen3-embedding.vllm.svc.cluster.local:8000`, and `local-small`, mapped to `qwen3.8-27b` at `qwen3-8-27b-llama-cpp.llama-cpp.svc.cluster.local:8000`. Local policies do not duplicate model sampling defaults. Prefix policies remove `openai/`, `anthropic/`, or `codex/` before forwarding.
