...
| PAC repository enrollment | Pantheon configuration contains `rfhold/kokoro-server`, `rfhold/whisperx-server`, `rfhold/smarthome-mcp`, and `rfhold/esp-wifi-cam`; it omits the replaced `rfhold/kokoro` and `rfhold/whisperx` names. The component creates one PAC resource per configured repository. | Apply the tracked stack change, then verify the live PAC resources and webhook discovery before claiming `rfhold/esp-wifi-cam` enrollment is operational. |
